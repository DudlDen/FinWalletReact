...
import {createExpenses} from "../http/expensesApi";
...
export function ModalAddExpenses({onClose,walletId}:IModalAddExpenses) {
    ...
    const click = async () => {
        try {
            const data = await createExpenses(walletId,name,value)
        ...
        <div className={styles.bcg}>
            <div className={styles.box} ref={ref}>
                <Form className="d-flex flex-column" onSubmit={submit}>
                    <div style={{fontFamily: "sans-serif",fontSize:16}}>Введите название расхода:</div>
                    <Form.Control
                        className="mt-3"
