...
export function ModalAddWallet({onClose}:IModalAddWallet) {
    ...
    const click = async () => {
        try {
            const data = await createWallet(name)
        ...
        <div className={styles.bcg}>
            <div className={styles.box} ref={ref}>
                <Form className="d-flex flex-column" onSubmit={submit}>
                    <div style={{fontFamily: "sans-serif",fontSize:16}}>Введите ваш название кошелька:</div>
                    <Form.Control
                        className="mt-3"
                        placeholder="Введите ваш название кошелька"
                        value={name}
                        onChange={nameChange}
                    />
